...
import Link from "next/link"; // Importe Link makhsoose Next.js ezafe shod
...
interface PortfolioProps {
  language: Language;
  darkMode: boolean;
  // onSelectProject ro optional (?) kardam chon dige be jaye Modal, mirim be page jadid
...
export const Portfolio: React.FC<PortfolioProps> = ({
  ...
  const [filter, setFilter] = useState<"all" | "app" | "package" | "dart">(
    "all",
  );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            ...
            >
              {/* Card Image Cover - Taghir be Link baraye SEO */}
              <Link
                href={`/projects/${project.id}`} // Masire page jadid
                className={`relative h-56 p-4 flex items-center justify-center overflow-hidden cursor-pointer block ${
                  darkMode ? "bg-slate-950/60" : "bg-slate-100"
                }`}
                title={`${t(project.titleKey)} Project by Ali Qelichbeigi`} // Title attribute baraye SEO
              >
                <img
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  {/* Title ham clickable shod */}
                  <Link href={`/projects/${project.id}`}>
                    <h3
                      className={`text-xl font-bold mb-2 transition-colors hover:text-[#20b2aa] ${
                        darkMode ? "text-white" : "text-slate-900"
                      }`}
                    >
                      {t(project.titleKey)}
                ...
                <div className="pt-4 border-t border-slate-800/40 flex items-center justify-between gap-2">
                  ...
                  <a
                    ...
                    title={`View ${t(project.titleKey)} on GitHub`} // Ezafe kardane Title baraye SEO
